    ...
    >
      ...
      <motion.rect
        ...
        transition={
          skip
            ? { duration: 0 }
            : { type: "spring", stiffness: 220, damping: 24, delay: 0 }
        }
      />

      {/* ── Right column — grows from bottom, slight stagger ── */}
      <motion.rect
        id="col-right"
        x={105}
        y={45}
        ...
        transition={
          skip
            ? { duration: 0 }
            : { type: "spring", stiffness: 220, damping: 24, delay: 0.13 }
        }
      />

      {/* ── Top bar — draws left to right ── */}
      <motion.path
        id="bar-top"
        d="M 30,93 L 130,80"
        stroke="url(#c8-grad)"
      ...
      {/* ── Bottom bar — draws left to right, slight delay ── */}
      <motion.path
        id="bar-bottom"
      ...
      {/* ── Wordmark — slides up + fades in after icon locks ── */}
      <motion.g
        id="wordmark"
        ...
        transition={
          skip
            ? { duration: 0 }
            : { duration: 0.55, delay: 0.88, ease: [0.16, 1, 0.3, 1] }
        }
      >
        <text
          x="158"
      ...
      {/* ── Tagline — last element to appear ── */}
      {showTagline && (
        <motion.g
          id="tagline"
