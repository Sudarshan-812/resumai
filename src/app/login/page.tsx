...
import {
  EnvelopeSimple as Mail, Lock, ArrowRight, Eye, EyeSlash as EyeOff, CheckCircle,
} from "@phosphor-icons/react";
import { CoinLoader } from "@/components/ui/coin-loader";
import { toast } from "sonner";
import { createClient } from "@/app/lib/supabase/client";
import { Button } from "@/components/ui/button";
import VivaLogo, { VivaMark } from "@/components/branding/VivaLogo";
...
const LoginPage: FC = (): JSX.Element => {
  ...
  return (
    <div className="grid min-h-dvh w-full bg-background lg:grid-cols-2">
      ...
      {/* ── Left panel - brand ── */}
      <div className="relative hidden overflow-hidden flex-col border-r border-border bg-muted/20 p-10 lg:flex">
          ...
          <div className="max-w-md space-y-8">
            ...
            >
              <h1 className="font-display text-5xl text-foreground leading-[1.05] tracking-tight">
                Get past the <br />
                <span className="text-primary">résumé screen.</span>
              </h1>
            ...
            <motion.div
              ...
              className="w-full rounded-2xl border border-border bg-background/60 p-6 shadow-xl shadow-primary/5"
            >
              <div className="flex items-center gap-3 mb-5">
                <VivaMark size={22} />
                <span className="text-[11px] font-mono uppercase tracking-widest text-muted-foreground">Match report</span>
                <span className="ml-auto text-xl font-bold font-mono text-primary">92<span className="text-xs text-muted-foreground">/100</span></span>
              </div>
              <div className="space-y-3">
                {[
                  ["Keyword match", 88],
                  ["Experience alignment", 95],
                  ["ATS-safe formatting", 90],
                ].map(([label, val]) => (
        ...
        <div className="mt-auto pt-10 border-t border-border/50">
          <span className="text-[11px] text-muted-foreground">© 2026 Viva</span>
        </div>
      </div>
        ...
        >
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {isSignUp && (
                <p className="ml-1 flex items-center gap-1.5 text-[11px] text-muted-foreground">
                  <CheckCircle size={12} className={password.length >= 8 ? "text-emerald-600" : "text-muted-foreground/40"} weight={password.length >= 8 ? "fill" : "regular"} />
