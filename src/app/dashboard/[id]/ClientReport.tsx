...
function CountUp({ to, delay = 0, suffix = "", size = 28, color = "#1c2024", onDone }: {
...
function BreakdownStat({ label, pct, color, delay }: { label: string; pct: number; color: string; delay: number }) {
  return (
    ...
    >
      <p className="text-[9px] font-mono uppercase tracking-[0.16em] mb-2" style={{ color: "#b9bbc6" }}>{label}</p>
      <CountUp to={pct} delay={delay} suffix="%" size={32} color={color} />
      <div className="mt-2.5 h-[3px] rounded-full overflow-hidden" style={{ background: "#d9d9e0" }}>
        <motion.div
  ...
  return (
    ...
    >
      <div className="h-14 shrink-0 flex items-center justify-between px-5"
        ...
        <div className="flex items-center gap-3">
          <motion.button onClick={onClose} whileHover={{ scale: 1.1, rotate: 90 }} whileTap={{ scale: 0.9 }}
            transition={SPRING}
            className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ color: "#80838d" }}>
            <X size={16} />
          </motion.button>
          <div>
            <p className="text-[14px] font-semibold leading-none" style={{ color: "#1c2024" }}>Resume Copilot</p>
            <p className="text-[10px] mt-0.5" style={{ color: "#80838d" }}>Your resume · left  ·  chat · right</p>
          </div>
        </div>
          ...
          <AnimatePresence>
            {isAiLoading && (
              <motion.div initial={{ opacity: 0, x: 6 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 6 }}
                className="hidden sm:flex items-center gap-1.5 text-[11px] font-medium" style={{ color: "#12a594" }}>
                <motion.span className="w-1.5 h-1.5 rounded-full" style={{ background: "#12a594" }}
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 overflow-hidden">
        <div className="hidden md:flex flex-col overflow-hidden" style={{ borderRight: "1px solid #d9d9e0" }}>
    ...
    <DashboardShell>
      <div style={{ background: "#f9f9fb", minHeight: "100%" }}>
        ...
        <div style={{ background: "#FFFFFF", borderBottom: "1px solid #d9d9e0" }}>
          <div className="max-w-3xl mx-auto px-6 md:px-10">
            ...
            <motion.div
              ...
              style={{ borderBottom: "1px solid #f0f0f3" }}
            >
              <Link href="/dashboard"
              ...
              <div className="flex items-center gap-2">
                ...
                <motion.button onClick={handleCopy}
                  ...
                  style={copied
                    ...
                    : { border: "1px solid #d9d9e0", color: "#80838d" }}>
            ...
            <div className="flex items-start justify-between gap-6 pt-7 pb-6">
              ...
              >
                <p className="text-[9px] font-mono uppercase tracking-[0.22em] mb-3" style={{ color: "#b9bbc6" }}>
                  ATS Report
                </p>
                <h1
                  className="font-display font-semibold tracking-tight capitalize"
                  style={{ color: "#1c2024", fontSize: "clamp(19px, 3.2vw, 28px)", lineHeight: 1.22 }}
                >
                  {fileName}
                </h1>
                <p className="mt-3 text-[13px] leading-[1.75] line-clamp-2" style={{ color: "#80838d" }}>
                  {feedback}
                </p>
              </motion.div>
              ...
              >
                ...
                {/* Progress bar */}
                <div className="w-24 h-[3px] rounded-full overflow-hidden" style={{ background: "#f0f0f3" }}>
                  <motion.div
                    className="h-full rounded-full"
                    initial={{ width: 0 }}
                    animate={{ width: `${score}%` }}
            ...
            >
              {[
                ...
                >
                  ...
                  {i < 3 && <span className="text-[10px] px-1" style={{ color: "#d9d9e0" }}>·</span>}
                </motion.div>
              ))}
            </motion.div>
          </div>
        ...
        <div className="max-w-3xl mx-auto px-6 md:px-10 py-8">
          ...
          <div style={{ height: 1, background: "#d9d9e0", marginBottom: 32 }} />
          ...
          >
            <div className="flex items-center gap-6 mb-6" style={{ borderBottom: "1px solid #d9d9e0" }}>
              {TABS.map(t => (
                <button key={t.id} onClick={() => setTab(t.id)}
                  className="relative pb-3.5 text-[12px] font-semibold transition-colors flex items-center gap-1.5"
                  style={{ color: tab === t.id ? "#1c2024" : "#80838d" }}>
                  {t.label}
                  {t.count !== undefined && (
                    <motion.span
                      layout
                      ...
                      style={{
                        background: t.id === "formatting" && issues.length > 0 ? "rgba(225,29,72,0.10)" : "rgba(18,165,148,0.10)",
                        color:      t.id === "formatting" && issues.length > 0 ? "#e11d48"               : "#12a594",
                      }}>
                      {t.count}
                    </motion.span>
                  )}
            ...
            <AnimatePresence mode="wait">
              ...
              {tab === "summary" && (
                <motion.div key="s"
                  ...
                  className="pb-4"
                >
                  <p className="text-[14px] leading-[1.9]" style={{ color: "#60646c" }}>{feedback}</p>
                </motion.div>
              )}
                ...
                >
                  <div>
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <span className="text-[10px] font-mono uppercase tracking-[0.16em]" style={{ color: "#80838d" }}>
                        Matched — {skills.length}
                      </span>
                    </div>
                    {skills.length > 0
                      ? <div className="flex flex-wrap gap-2">{skills.map((s, i) => <Chip key={i} label={s} variant="match" index={i} />)}</div>
                      : <p className="text-[13px]" style={{ color: "#80838d" }}>No skills matched.</p>
                    }
                  </div>
                  <div style={{ height: 1, background: "#d9d9e0" }} />
                  <div>
                    <div className="flex items-center gap-2 mb-4">
                    ...
                    {missing.length > 0
                      ...
                      : <p className="text-[13px]" style={{ color: "#80838d" }}>Nothing missing — great coverage.</p>
                    }
                  </div>
                </motion.div>
              )}
                ...
                >
                  {issues.length > 0 ? issues.map((iss, i) => (
                    <motion.div key={i}
                      ...
                      style={{ borderBottom: i < issues.length - 1 ? "1px solid #d9d9e0" : "none" }}
                    >
                      <motion.div
                      ...
                      <div>
                        ...
                        <p className="text-[13.5px] leading-relaxed" style={{ color: "#60646c" }}>{iss}</p>
                      </div>
                    </motion.div>
                  )) : (
                    <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }}
                      ...
                      <div>
                        <p className="text-[14px] font-semibold" style={{ color: "#1c2024" }}>Formatting looks clean</p>
                        <p className="text-[12px] mt-0.5" style={{ color: "#80838d" }}>Passed all ATS formatting checks.</p>
                      </div>
                    </motion.div>
                  )}
                </motion.div>
